...
import { sampleVitalityCurve, SEED_PATTERNS, SEED_PATTERNS_HEX, type SeedPattern, type SeedPatternId } from '@games-of-life/core';
import type { Simulation } from '@games-of-life/webgpu';

// Simulation state
let isPlaying = $state(true); // Start playing by default
let isRecording = $state(false);
let speed = $state(1); // Steps per second — default to 1 so NLCA playback paces deliberately
...
// Seed patterns are canonicalized in @games-of-life/core.
...
export function getSimulationState() {
	return {
		...
		set speed(value: number) {
			...
			speed = Math.max(1, Math.min(1000, value));
		},
